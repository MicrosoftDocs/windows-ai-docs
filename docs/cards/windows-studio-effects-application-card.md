...
Despite its optimizations, Windows Studio Effects is subject to several common limitations that can affect performance and user experience across all features. Understanding Studio Effect’s limitations is crucial to determine if it is used within safe and effective boundaries, we encourage users to refer to either the Microsoft [Enterprise AI Services Code of Conduct (for organizations)](/legal/ai-code-of-conduct) or the [Code Conduct section in the Microsoft Services Agreement (for individuals)](https://www.microsoft.com/servicesagreement#3_codeOfConduct) as well as the following considerations when choosing a use case: 
