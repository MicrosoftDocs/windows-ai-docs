...
| **NPU (Neural Processing Unit)** | A specialized chip designed to speed up AI workloads while using less power than a CPU or GPU. Text Recognition (OCR) can run on the NPU for fast, power-efficient on-device processing where available. |
