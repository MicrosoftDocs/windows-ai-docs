...
# What Windows AI APIs are available today?

Windows Copilot Runtime includes the following features and AI-backed APIs powered by models running locally on the Windows device. To get started trying available APIs, see [Get Started with Windows AI APIs](mindex.md), this guidance includes code to check whether the required models are available on the user's device.

These APIs ship in the [Windows App SDK](/windows/apps/windows-app-sdk/), and are currently only available in the latest [experimental channel release of the Windows App SDK](/windows/apps/windows-app-sdk/experimental-channel).
